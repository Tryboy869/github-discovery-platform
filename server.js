// server.js - Backend Service avec Turso DB (NEXUS AXION 4.1 - BigInt Fix)
...
export class BackendService {
  ...
  // ========== HELPER : CONVERTIR BIGINT EN STRING ==========
  sanitizeRow(row) {
    if (!row) return row;
  ...
  async init() {
    ...
    try {
      ...
      try {
        ...
        console.log('⚠️ [BACKEND] No old users table to drop');
      }
      ...
      try {
        ...
        console.log('⚠️ [BACKEND] No old projects table to drop');
      }
  ...
  // ========== RESET DATABASE (FONCTION INTÉGRÉE) ==========
  async resetDatabase() {
    console.log('🔄 [BACKEND] Resetting databases...');
    
    try {
      // Supprimer toutes les données
      await this.dbUsers.execute('DELETE FROM users');
      console.log('✅ [BACKEND] Users table cleared');
      
      await this.dbProjects.execute('DELETE FROM projects');
      console.log('✅ [BACKEND] Projects table cleared');
      
      // Reset auto-increment
      await this.dbUsers.execute('DELETE FROM sqlite_sequence WHERE name="users"');
      await this.dbProjects.execute('DELETE FROM sqlite_sequence WHERE name="projects"');
      
      console.log('✅ [BACKEND] Database reset completed');
  ...
  async signup(email, username, password) {
    ...
    try {
      ...
      // FIX BIGINT: Convertir en string
      const userId = result.lastInsertRowid.toString();
      console.log('[BACKEND] User inserted, ID:', userId);
  ...
  async login(email, password) {
    ...
    try {
      ...
      // FIX BIGINT: Sanitize row
      const user = this.sanitizeRow(result.rows[0]);
  ...
  async getProjects(filters = {}) {
    ...
    try {
      ...
      // FIX BIGINT: Sanitize all rows
      const sanitizedRows = this.sanitizeRows(result.rows);
      
      return {
        success: true,
        projects: sanitizedRows.map(row => ({
          ...row,
  ...
  async getProject(id) {
    ...
    try {
      ...
      // FIX BIGINT: Sanitize row
      const project = this.sanitizeRow(result.rows[0]);
  ...
  async getStats() {
    ...
    try {
      ...
      // FIX BIGINT: Sanitize rows
      return {
        success: true,
        stats: {
          total_projects: Number(totalResult.rows[0].total),
          by_language: this.sanitizeRows(langResult.rows),
          by_category: this.sanitizeRows(catResult.rows)
        }
      };
    } catch (error) {
